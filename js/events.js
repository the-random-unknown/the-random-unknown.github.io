...
kazoni_voda[3] = async function () {
	new Battle(6, 5, async function () {
		await output.write("V troskách lodi se žádný materiál nenachází. Boj tě stál jednu jednotku paliva a poškodil sis karmu s Ocampy")
...
kazoni_voda[6] = async function () {
	new Battle(10, 5, async function () {
		...
		if (inventory.getScrap() > 2) {
			output.createButton("Letíš dolů", kazoni_voda[7]);
			inventory.addFuel(-2);
